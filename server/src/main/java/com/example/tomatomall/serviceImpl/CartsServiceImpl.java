...
public class CartsServiceImpl implements CartsService {
    ...
    public CheckoutResultVO checkout(CheckoutRequestVO request) {
        ...
        try {
            for (String cartItemId : cartItemIds) {
                productRepository.lockStock(cartsRepository.findByCartItemId(Integer.valueOf(cartItemId)).getProduct().getId(),
            ...
            for (String sucCartItemId : sucCartItemIds) {
                productRepository.releaseStock(cartsRepository.findByCartItemId(Integer.valueOf(sucCartItemId)).getProduct().getId(),
    ...
    public CheckoutResultVO cancelOrder(String orderId) {
        ...
        for (OrderDetail detail : orderDetails) {
            productRepository.releaseStock(detail.getProduct().getId(), detail.getCarts().getQuantity());
        }
